...
- 服务端统一串行执行工具调用，并对重复查询、重复结果、重复读取、连续无效调用和只搜索不读取进行收敛控制；无来源时即使模型主动结束，也不能生成自由回答或伪脚注。
...
- 当前发布版本为 `20260801T074810Z`，包含 `PF1E` 与 `Pathfinder 1E` 显式别名。
- 85 题混合检索回归为 Hit@5 96.5%、MRR 0.8210。
- 《夕妖晚谣》1.2 中文规则库已发布 154 个父文档、1271 个检索子块。
- 当前发布版本为 `20260801T074827Z`，包含 `GSS` 与 `Golden Sky Stories` 显式别名。
- 14 题混合检索验收为 Hit@5 100%、MRR 0.9107；纯向量为 Hit@5 92.9%、MRR 0.7405。
